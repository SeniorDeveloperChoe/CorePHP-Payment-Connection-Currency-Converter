	...
	public function getPageContent() 
	{
		...
		$email=$password=$rememberme="";
		 ...
		 if(isset($_COOKIE['rememberme'])){
            ...
            $password = isset($_COOKIE['password']) ? custom_decoder($_COOKIE['password']) : '';
            $rememberme = isset($_COOKIE['rememberme']) ? 'checked' : '';
        }
		...
		extract($data);
		 ...
		 if (isset($email) && isset($password)) {
            ...
            if ($selQuery->affectedRows() >= 1) {
                ...
                if ($result != false) {
                    extract($result);
                    /*unset the temp_user id*/
                    
                    if(isset($_SESSION['temp_userid'])){
                        $this->db->query("update tbl_cart set user_id =".$result['id']." , tmp_userid='0' where tmp_userid =".$_SESSION['temp_userid']);
                    ...
                    if ($email_veri_status == "n" && $isActive == "n") {
                        ...
                        if(isset($_SESSION['redirect']) && $_SESSION['redirect'] == 'login_to_continue' ){
